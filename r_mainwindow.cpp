...
// ============================================================================
StimulationPage::StimulationPage(QWidget *parent,TreatmentManager *manager ) : QWidget(parent), m_manager(manager)
{
    setupUi();
    ...
    if (m_manager) 
    {
    connect(m_manager,&TreatmentManager::timeUpdated,this,&StimulationPage::timehandle);
    connect(m_manager, &TreatmentManager::runstateChanged, this, &StimulationPage::onStateChanged);
    onSliderMoved();
    onBtnApplyClicked();
    }
}
...
void StimulationPage::onBtnApplyClicked() {
    // 1. 将滑块值保存到 Active 变量
    sti_param.freq = m_slFreq->value();
    sti_param.posAmp = m_slPosAmp->value() / 10.0;
    sti_param.negAmp = m_slNegAmp->value() / 10.0;
    sti_param.posW = m_slPosW->value();
    sti_param.dead = m_slDead->value();
    sti_param.negW = m_slNegW->value();
    ...
    // 3. 联动 PID 页面
    emit posAmpChanged(sti_param.posAmp);
...
void StimulationPage::timehandle(int secondsLeft)
{
    m_remainingTime=secondsLeft;
    int m = m_remainingTime / 60;
    int s = m_remainingTime % 60;
     m_lblTime->setText(QString("%1:%2").arg(m, 2, 10, QChar('0')).arg(s, 2, 10, QChar('0')));
...
void StimulationPage::toggleStimulation() {
     Runstate state = m_manager->currentState();
    if(state == Runstate::Running) {

        m_manager->stopTreatment();

    } else {


        m_manager->updateParameters(sti_param);
        m_manager->startTreatment(m_activeDur);
      
    }
}
void StimulationPage::onStateChanged(Runstate state)
{
    if (state == Runstate::Running) {
        // 变成红色停止样式
        ...
        m_btnApply->setEnabled(false);// 运行时禁止改参数
...
// ============================================================================
PidPage::PidPage(QWidget *parent,TreatmentManager *manager) : QWidget(parent),m_manager(manager)
{
    setupUi();
    onSliderChanged();
}
...
void PidPage::setupUi() {
    ...
    // 确认参数按钮
    m_btnApply = new QPushButton("确认参数");
    m_btnApply->setFixedSize(140, 40);
    m_btnApply->setStyleSheet("QPushButton { background: #00b8d4; color: white; border: none; border-radius: 6px; font-weight: bold; } QPushButton:hover { background: #00e5ff; } QPushButton:pressed { background: #0097a7; }");
    connect(m_btnApply, &QPushButton::clicked, this, &PidPage::onBtnApplyClicked);
    
    // 恢复默认按钮
    m_btnReset = new QPushButton("恢复默认");
    m_btnReset->setFixedSize(140, 40);
    m_btnReset->setCursor(Qt::PointingHandCursor);
    m_btnReset->setStyleSheet("QPushButton { background: #4ff103ff; color: #333; border: 1px solid #ccc; border-radius: 6px; font-weight: bold; } QPushButton:hover { background: #f5f5f5; }");
    connect(m_btnReset, &QPushButton::clicked, this, &PidPage::resetParams);
...
void PidPage::onSliderChanged() {
    ...
    m_valKp->setText(QString::number(m_slKp->value() / 10.0, 'f', 1));
    m_valKi->setText(QString::number(m_slKi->value() / 100.0, 'f', 2));
    m_valKd->setText(QString::number(m_slKd->value() / 100.0, 'f', 2));

}
void PidPage::onBtnApplyClicked()
{
    pid_param.kp = m_slKp->value() / 10.0;
    pid_param.ki = m_slKi->value() / 100.0;
    pid_param.kd = m_slKd->value() / 100.0;
    m_manager->setPIDParameters(pid_param);
...
MainWindow::MainWindow(QWidget *parent)
    : QMainWindow(parent)
{
    IBackend *backend = new RPMsgBackend(this);
    m_manager = new TreatmentManager(backend, this);
...
void MainWindow::setupUi() {
    ...
    m_pageStim = new StimulationPage(this, m_manager);
    m_pagePid = new PidPage(this, m_manager);
